...
# MAGIC %md
# MAGIC # Visualized Parsed Data with bboxes
# MAGIC
# MAGIC - Use the pull-down menu to select the file for visualization

# COMMAND ----------

# MAGIC %md
# MAGIC ## Read Config File to Collect Table Names
...
# MAGIC %md
# MAGIC ## Create a Pull-Down Menu

# COMMAND ----------

result_df = spark.table(config['raw_files_table_name'])

# COMMAND ----------

dbutils.widgets.dropdown(
    name='path_dropdown',
    defaultValue=result_df.select('path').first()['path'],
    choices=[row['path'] for row in result_df.select('path').distinct().collect()],
    label='Select Path'
)

# COMMAND ----------

selected_pdf = dbutils.widgets.get('path_dropdown')
print(f"Select {selected_pdf}")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Display Bounding Box on top of pages
...
SELECT
*
from {config['raw_files_table_name']}
where path = '{selected_pdf}'
"""

parsed_results = [row.parsed for row in spark.sql(query).collect()]
